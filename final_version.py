...
# создание базы данных с пользователям
...
# создание базы данных для отслеживания взаимосвязи босса и менеджера
...
# создание базы данных для статистики
...
passwords_for_tests = [1111, 1112, 1113, 1121, 1122, 1123, 1131, 1132, 1133, 1141, 1142, 1143,
                               1211, 1212, 1213, 1221, 1222, 1223, 1231, 1232, 1233, 1241, 1242, 1243,
                               1311, 1312, 1312, 1321, 1322, 1323, 1331, 1332, 1333, 1341, 1342, 1343,
                               2111, 2112, 2113, 2121, 2122, 2123, 2131, 2132, 2133, 2141, 2142, 2143,
                               2211, 2212, 2213, 2221, 2222, 2223, 2231, 2232, 2233, 2241, 2242, 2243,
                               2311, 2312, 2312, 2321, 2322, 2323, 2331, 2332, 2333, 2341, 2342, 2343,]
    ...
    if message.text == "/start":
        ...
        for value in cursor.execute('SELECT * FROM users WHERE user_id=?', (message.from_user.id,)):
            if value[2] == 'boss':
                ...
                bot.send_message(message.from_user.id, 'Заполните пожалуйста форму', reply_markup=murkup)
            else:
                user_id = message.from_user.id
                requests.post('http://127.0.0.1:8080/update', json={"user_id": user_id})
...
def handle_poll_answer(poll_answer):
    ...
    if user_id_to_tests_options[poll_answer.user.id][0]['test_id'] == 25 and \
        ...
        # mutex.acquire()
        result = user_id_to_tests_options[poll_answer.user.id][0]['result']
        bot.send_message(poll_answer.user.id, f'Вы набрали {result} баллов из 24')
        # mutex.release()
...
# Комментарий к переменной test: 1000-категория b2b, 2000-категория b2c
