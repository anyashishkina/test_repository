...
def db_table_val1(user_id: int, user_name: str, user_status: str, username: str, level: str, b2b_or_b2c: str):
...
def callback_worker(call):
    if call.data == "manager" or "boss":
        if call.data == "manager":
            ...
            if len(info_user_to) == 0:
                ...
                db_table_val1(user_id=us_id, user_name=us_name, user_status=status, username=username, level = '', b2b_or_b2c = '')
                if status == 'manager':
    ...
    if call.data == "typeofclientb" or call.data == "typeofclientc":
        if call.data == "typeofclientb":
            cursor.execute('UPDATE users SET b2b_or_b2c=? WHERE user_id=?', ("b2b", call.from_user.id,))
            db_users.commit()
            ...
            cursor.execute('UPDATE users SET b2b_or_b2c=? WHERE user_id=?', ("b2c", call.from_user.id,))
            db_users.commit()
...
def update_data():
    ...
    #     if value[4] == 'Новичок' and value[5] == 'b2c':
    test_values = {
